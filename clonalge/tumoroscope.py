...
import scipy
from . import simulation_tumoroscope as sim
...
class tumoroscope:
    ...
    def __init__(self,name, K, S, r, p, I, avarage_clone_in_spot,F,C,A,D,F_epsilon,optimal_rate,n_lambda,gamma,pi_2D,
        result_txt, rp_est_method='tum', inits=None):
        self.name = name
        self.K=K
        self.S=S

        if p is None or r is None:
            if rp_est_method == 'tum':
                adjust = np.tile(n_lambda, (I, 1))
                phi_samples = np.mean(D/(adjust.astype(float)),axis=1)#,(K,1)).transpose()
                phi_samples[phi_samples==0] = np.min(phi_samples[phi_samples>0])
                N = I
    ...
    def gibbs_sampling(self,seed,min_iter,max_iter,burn_in,batch,simulated_data,n_sampling,F_fraction,theta_variable,pi_2D,th,every_n_sample,changes_batch,var_calculation):
        ...
        iter = 0
        PZ = 0
        inits = self.initialization(K = self.K,S = self.S,r = self.r,p = self.p,I = self.I,
        ...
        if self.inits is None:
            n,H,G,pi,phi,Z,h_theta = inits
        else:
            n,H,G,pi,phi,Z,h_theta = self.inits
